...
export function StudentPlanSection({ alumno, isSuperAdmin, onRefresh, onAlumnoUpdate, openOverduePreviewToken }: Props) {
  ...
  const getEffStatus = (s: SuscripcionData) => getEffectiveSubStatus({ estado: s.estado, fecha_fin: getSubStatusEndDate(s), cancelada_at: s.cancelada_at, cancelada_motivo: s.cancelada_motivo, mp_status: (s as any).mp_status, origen_registro: (s as any).origen_registro });
