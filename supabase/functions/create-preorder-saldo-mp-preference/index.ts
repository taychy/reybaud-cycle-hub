...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: preorder } = await supabaseAdmin
      ...
      .select("id, alumno_id, product_id, producto_nombre, moneda, saldo_pendiente, estado_pago_sena")
      .eq("id", preorder_id)
      .maybeSingle();
    ...
    if (preorder.estado_pago_sena !== "confirmada") {
      return new Response(JSON.stringify({ error: "Primero hay que pagar la seña" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    const saldo = Number(preorder.saldo_pendiente);
    if (!saldo || saldo <= 0) {
    ...
    const preferenceBody = {
      items: [{
        title: `Saldo preventa - ${preorder.producto_nombre}`,
        quantity: 1,
        unit_price: saldo,
      ...
      back_urls: {
        success: `${origin}/pago-resultado?status=approved&kind=preorder_saldo`,
        failure: `${origin}/pago-resultado?status=failure&kind=preorder_saldo`,
        pending: `${origin}/pago-resultado?status=pending&kind=preorder_saldo`,
      },
      auto_return: "approved",
      external_reference: `preorder_saldo:${preorder.id}`,
      metadata: {
        payment_type: "preorder_saldo",
    ...
    if (!mpResponse.ok) {
      console.error("[create-preorder-saldo-mp-preference] MP error:", JSON.stringify(mpData));
    ...
    console.error("create-preorder-saldo-mp-preference error:", err);
