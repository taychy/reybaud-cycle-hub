...
SELECT
  ...
  ('Pago plan: '::text || COALESCE(p.nombre, '—'::text)) ||
    CASE WHEN s.metodo_pago IS NOT NULL THEN (' ('::text || s.metodo_pago) || ')'::text ELSE ''::text END AS concepto,
  'suscripciones'::text AS fuente_tabla,
  s.id AS fuente_id,
  0::numeric AS debe,
...
WHERE s.cancelada_at IS NULL
  ...
  AND (s.estado = ANY (ARRAY['activa'::text, 'pendiente_verificacion'::text, 'vencida'::text, 'conciliado'::text]))
  AND (s.origen_registro = ANY (ARRAY['automatico'::text, 'cargado_admin'::text]))

UNION ALL
-- ============ EVENTOS / VIAJES: cargo único por reserva ============
SELECT
  er.alumno_id,
  COALESCE(er.confirmed_at::date, er.created_at::date) AS fecha,
  'cargo_reserva'::text AS tipo,
  COALESCE(e.title, 'Evento'::text)
    || CASE WHEN er.package_nombre_snapshot IS NOT NULL
            THEN ' — '::text || er.package_nombre_snapshot
...
WHERE rp.alumno_id IS NOT NULL
  AND rp.status = 'validado'::text
  AND rp.anulado_at IS NULL

UNION ALL
-- ============ PREVENTAS: cargo total al crearse ============
SELECT
  sp.alumno_id,
  ...
  sp.id AS fuente_id,
  COALESCE(sp.precio_total, sp.precio_unitario * COALESCE(sp.cantidad, 1), 0::numeric) AS debe,
  0::numeric AS haber,
  ...
  jsonb_build_object(
    ...
    'sena_monto', sp.sena_monto,
    'saldo_pendiente', sp.saldo_pendiente,
    'estado_pago_sena', sp.estado_pago_sena
  ) AS referencia_extra
FROM store_preorders sp
WHERE sp.alumno_id IS NOT NULL
  AND sp.cancelada_at IS NULL
  AND COALESCE(sp.estado, '') <> 'cancelada'

UNION ALL
-- ============ PREVENTAS: pago de seña (haber) ============
SELECT
  sp.alumno_id,
...
WHERE sp.alumno_id IS NOT NULL
  AND sp.cancelada_at IS NULL
  AND sp.estado_pago_sena = ANY (ARRAY['pagado'::text, 'aprobado'::text, 'pagada'::text])
  AND COALESCE(sp.sena_monto, 0) > 0

UNION ALL
-- ============ PREVENTAS: pago del saldo final (haber) ============
SELECT
  sp.alumno_id,
...
WHERE so.alumno_id IS NOT NULL
  AND COALESCE(so.status, '') <> 'cancelada'

UNION ALL
-- ============ TIENDA (órdenes): pago (haber) ============
SELECT
  so.alumno_id,
...
FROM store_orders so
WHERE so.alumno_id IS NOT NULL
  AND (so.pagado_at IS NOT NULL OR so.status = ANY (ARRAY['pagada'::text, 'pagado'::text, 'completada'::text, 'entregada'::text]))
