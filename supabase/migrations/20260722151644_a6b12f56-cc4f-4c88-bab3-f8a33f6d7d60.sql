...
UNION ALL
 SELECT s.alumno_id,
    ...
    s.id AS fuente_id,
    0::numeric AS debe,
    -- Para MP: sumar lo realmente acreditado en mp_account_movements.
    -- Para otros métodos manuales: precio_final del plan.
    CASE
      WHEN s.metodo_pago = 'mercadopago'::text THEN COALESCE(
        (SELECT SUM(mp.amount)
           FROM public.mp_account_movements mp
          WHERE mp.suscripcion_id = s.id
  ...
  WHERE s.cancelada_at IS NULL
    ...
    -- Ocultar el renglón "Pago plan" si es MP y todavía no hay ningún cobro acreditado.
    AND (
      s.metodo_pago <> 'mercadopago'::text
      OR EXISTS (
