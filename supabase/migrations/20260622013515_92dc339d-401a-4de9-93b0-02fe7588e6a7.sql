...
-- Agrega información de medio de pago a los ajustes manuales de cuenta corriente
ALTER TABLE public.cuenta_ajustes
  ADD COLUMN IF NOT EXISTS medio_pago text,
  ADD COLUMN IF NOT EXISTS cuenta_mp_id uuid REFERENCES public.cuentas_mp(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS referencia_externa text;

-- Recreate view to expose medio_pago / cuenta / referencia en ambos tipos de ajuste
...
UNION ALL
 SELECT s.alumno_id,
    COALESCE(
        CASE WHEN s.origen_registro = ANY (ARRAY['automatico'::text, 'cargado_admin'::text])
             THEN s.fecha_inicio ELSE NULL::date
...
UNION ALL
 SELECT sp.alumno_id,
  ...
  WHERE sp.alumno_id IS NOT NULL AND sp.cancelada_at IS NULL
    AND (sp.estado_pago_sena = ANY (ARRAY['pagado'::text, 'aprobado'::text, 'pagada'::text, 'confirmada'::text, 'pendiente_verificacion'::text]))
    AND COALESCE(sp.sena_monto, 0::numeric) > 0::numeric
UNION ALL
 SELECT sp.alumno_id,
   ...
   FROM store_preorders sp
  WHERE sp.alumno_id IS NOT NULL AND sp.cancelada_at IS NULL
    AND COALESCE(sp.saldo_pendiente, 0::numeric) <= 0::numeric
    AND COALESCE(sp.precio_total, 0::numeric) > COALESCE(sp.sena_monto, 0::numeric)
UNION ALL
 SELECT so.alumno_id,
