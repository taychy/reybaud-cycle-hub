...
-- Housekeeping: cerrar las 111 subs "activa" vencidas. Deshabilitamos temporalmente
-- el guard de alumno porque esta operación es administrativa/mantenimiento.
...
-- Endurecer el chequeo de duplicados: auto-heal de subs "activa" vencidas del
-- mismo alumno antes de decidir si hay conflicto. Corre como SECURITY DEFINER
-- (postgres owner) por lo que el guard de alumno no aplica dentro del trigger.
...
BEGIN
  ...
  -- (excepto pausas). Evita depender del frontend o del cron.
