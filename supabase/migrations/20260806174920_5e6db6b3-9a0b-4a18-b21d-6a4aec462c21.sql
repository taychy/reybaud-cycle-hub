...
DROP FUNCTION IF EXISTS public.merge_alumnos(uuid, uuid);

CREATE OR REPLACE FUNCTION public.preview_merge_alumnos(_principal_id uuid, _duplicado_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    EXECUTE format('SELECT count(*) FROM %s WHERE %I = $1', r.tbl, r.col)
      INTO cnt USING _duplicado_id;
    IF cnt > 0 THEN
      total := total + cnt;
      detalle := detalle || jsonb_build_object('tabla', r.tbl, 'columna', r.col, 'registros', cnt);
    END IF;
  END LOOP;

  RETURN jsonb_build_object('total_registros', total, 'detalle', detalle);
END;
$$;
...
REVOKE ALL ON FUNCTION public.preview_merge_alumnos(uuid, uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.merge_alumnos(uuid, uuid) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.preview_merge_alumnos(uuid, uuid) TO authenticated, service_role;
