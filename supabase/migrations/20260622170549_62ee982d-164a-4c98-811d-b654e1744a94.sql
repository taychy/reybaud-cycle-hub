...
BEGIN
  ...
  cuotas AS (
    SELECT
      ...
      ri.status AS estado,
      true AS mp_disponible,
      jsonb_build_object('reservation_id', ri.reservation_id, 'amount', ri.balance_due, 'installment_id', ri.id) AS payment_payload
...
REVOKE ALL ON FUNCTION public.get_cuenta_publica(uuid, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_cuenta_publica(uuid, text, text) TO anon, authenticated, service_role;