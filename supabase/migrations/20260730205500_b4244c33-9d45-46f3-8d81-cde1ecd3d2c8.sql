...
UNION ALL
 SELECT s.alumno_id,
    ...
    ('Pago plan: '::text || COALESCE(p.nombre, '—'::text)) ||
        CASE
            WHEN s.metodo_pago = 'saldo_a_favor'::text AND EXISTS (
                   SELECT 1 FROM cuenta_ajustes ca
                    WHERE ca.tipo = 'credito'::text
                      AND ca.aplicado_a_fuente_tabla = 'suscripciones'::text
                      AND ca.aplicado_a_fuente_id = s.id)
              THEN ' (saldo a favor aplicado)'::text
        ...
        CASE
            WHEN s.metodo_pago = 'saldo_a_favor'::text AND EXISTS (
                   SELECT 1 FROM cuenta_ajustes ca
                    WHERE ca.tipo = 'credito'::text
                      AND ca.aplicado_a_fuente_tabla = 'suscripciones'::text
                      AND ca.aplicado_a_fuente_id = s.id) THEN 0::numeric
   ...
   FROM suscripciones s
     LEFT JOIN planes p ON p.id = s.plan_id
  WHERE s.cancelada_at IS NULL AND s.metodo_pago IS NOT NULL AND (s.estado = ANY (ARRAY['activa'::text, 'pendiente_verificacion'::text, 'vencida'::text, 'finalizada'::text, 'conciliado'::text])) AND (s.origen_registro = ANY (ARRAY['automatico'::text, 'cargado_admin'::text])) AND (s.metodo_pago <> 'mercadopago'::text OR (EXISTS ( SELECT 1
           FROM mp_account_movements mp
