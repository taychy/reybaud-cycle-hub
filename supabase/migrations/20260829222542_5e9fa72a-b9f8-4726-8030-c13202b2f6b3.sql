...
-- Helper: reconcilia la tarea de WhatsApp por cambio de grupo
...
BEGIN
  ...
  -- Nombre del actor (nunca falla: 'Sistema' si no hay usuario)
  IF v_uid IS NOT NULL THEN
    ...
    IF v_actor IS NULL THEN
      SELECT COALESCE(NULLIF(ap.nombre_completo, ''), ap.email)
  ...
  -- Volvió al grupo ya sincronizado
...
-- Trigger: cualquier cambio real de alumnos.grupo reconcilia la tarea
...
BEGIN
  IF NEW.grupo IS DISTINCT FROM OLD.grupo THEN
    PERFORM public.reconciliar_tarea_whatsapp_grupo(NEW.id, OLD.grupo, NEW.grupo, auth.uid());
  END IF;
  RETURN NULL;
END;
$function$;

DROP TRIGGER IF EXISTS trg_alumnos_grupo_whatsapp_sync ON public.alumnos;
CREATE TRIGGER trg_alumnos_grupo_whatsapp_sync
AFTER UPDATE OF grupo ON public.alumnos
FOR EACH ROW
WHEN (OLD.grupo IS DISTINCT FROM NEW.grupo)
EXECUTE FUNCTION public.trg_alumnos_grupo_whatsapp_sync();

-- RPC: sólo valida permisos y actualiza el grupo (el trigger reconcilia)
CREATE OR REPLACE FUNCTION public.registrar_cambio_grupo_alumno(p_alumno_id uuid, p_nuevo_grupo text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF p_nuevo_grupo IS NOT DISTINCT FROM v_prev THEN
    RETURN jsonb_build_object('accion', 'sin_cambio', 'grupo_destino', p_nuevo_grupo);
  END IF;
