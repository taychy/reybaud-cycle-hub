-- Fix: include payments for paid subs even when they expired or were conciliated.
-- The previous version only counted payments while estado IN ('activa','pendiente_verificacion'),
-- which made historical paid subs disappear from cuenta corriente once vencidas.
...
UNION ALL
 SELECT s.alumno_id,
    COALESCE(
      CASE WHEN s.origen_registro IN ('automatico','cargado_admin') THEN s.fecha_inicio END,
      (s.updated_at)::date
    ) AS fecha,
...
UNION ALL
 SELECT er.alumno_id,
    ...
    COALESCE(e.title, 'Evento'::text) ||
        CASE WHEN ri.label IS NOT NULL THEN ' — '::text || ri.label
             ELSE ' — Cuota '::text || ri.installment_number::text END AS concepto,
...
GRANT SELECT ON public.vw_cuenta_corriente_movimientos TO authenticated, service_role;