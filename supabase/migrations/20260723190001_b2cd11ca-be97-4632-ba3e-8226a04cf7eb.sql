...
BEGIN
  ...
  LOOP
    ...
    -- order_id is intentionally NULL: the FK points to store_orders, and this movement
    -- originates from a supplier_order. The supplier reference is preserved in `motivo`.
    INSERT INTO public.stock_movements(product_id, variante, tipo, cantidad, stock_anterior, stock_nuevo, motivo, order_id)
    VALUES (_item.product_id, NULLIF(_variant_key,''), 'ingreso', _item.cantidad_recibida, _current_qty, _new_qty, 'ingreso_proveedor:' || _order.numero || ':' || _order_id::text, NULL);
