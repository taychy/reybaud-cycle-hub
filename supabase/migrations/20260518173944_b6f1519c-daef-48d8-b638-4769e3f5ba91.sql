...
BEGIN
  -- Helper: marcar hechas las tareas que cumplan un set de IDs
  -- 1. whatsapp_check: ya pasamos la ventana del mes actual (día > 17) o no estamos en rango
  ...
  -- 2. alumno_inactivo_30d: alumno ya fue actualizado o no está activo
  ...
  -- 3. coach_sin_feedback_14d: ya cargó feedback en los últimos 14 días
  UPDATE public.tareas t
  ...
  -- 4a. certificado_no_cargado: ya cargó o alumno inactivo
  UPDATE public.tareas t
  ...
  -- 4b. certificado_por_vencer: nueva fecha de vencimiento más lejana o quitado
  UPDATE public.tareas t
  ...
  -- 5. pago_pendiente_validar: suscripción ya no está pendiente
  UPDATE public.tareas t
  ...
  -- 6. suscripcion_pendiente_15d: suscripción salió del estado intermedio
  UPDATE public.tareas t
  ...
  -- 7. suscripcion_vencida_sin_renovar: alumno ya tiene una suscripción vigente o pasó a inactivo/bloqueado
  UPDATE public.tareas t
  ...
  -- 8. alumno_estado_intermedio_15d: alumno volvió a un estado terminal
  UPDATE public.tareas t
  ...
  -- 9. renovacion_proxima_7d: ya se renovó (nueva sub creada con fecha_fin posterior) o suscripción no está activa
  UPDATE public.tareas t
...
GRANT EXECUTE ON FUNCTION public.auto_resolve_tareas_automaticas() TO authenticated;

-- Modificar generate_tareas_automaticas para que primero auto-resuelva las ya solucionadas
...
DECLARE
  ...
  r record;
BEGIN
  -- 0. Auto-resolver tareas cuyas condiciones ya se cumplieron
  ...
  FOR r IN
    ...
    WHERE a.estado = 'activo' AND a.updated_at < (now() - interval '30 days')
  LOOP
  ...
  FOR r IN
    ...
    FROM public.coaches c WHERE c.estado = 'activo'
  LOOP
    IF r.last_fb IS NULL OR r.last_fb < (v_today - interval '14 days') THEN
      ...
      VALUES ('automatica', 'coach_sin_feedback_14d',
  ...
  FOR r IN
    SELECT id, nombre, apellido FROM public.alumnos
    WHERE estado = 'activo' AND COALESCE(medical_certificate_status, 'no_cargado') = 'no_cargado'
  LOOP
    INSERT INTO public.tareas (tipo, origen, titulo, descripcion, rol_destino, prioridad, entidad_tipo, entidad_id, dedupe_key, metadata)
  ...
  FOR r IN
    SELECT id, nombre, apellido, medical_certificate_expiration_date FROM public.alumnos
  ...
  FOR r IN
    SELECT s.id, s.alumno_id, a.nombre, a.apellido, s.updated_at FROM public.suscripciones s
    JOIN public.alumnos a ON a.id = s.alumno_id
    WHERE s.estado = 'pendiente_verificacion' AND s.updated_at < (now() - interval '48 hours')
  LOOP
    INSERT INTO public.tareas (tipo, origen, titulo, descripcion, rol_destino, prioridad, entidad_tipo, entidad_id, dedupe_key, metadata)
  ...
  FOR r IN
    SELECT s.id, s.alumno_id, a.nombre, a.apellido, s.estado, s.created_at FROM public.suscripciones s
    JOIN public.alumnos a ON a.id = s.alumno_id
    WHERE s.estado IN ('pendiente','pago_pendiente','pausa','acceso_pausado')
      AND s.cancelada_at IS NULL AND s.created_at < (now() - interval '24 hours')
  LOOP
    INSERT INTO public.tareas (tipo, origen, titulo, descripcion, rol_destino, prioridad, entidad_tipo, entidad_id, dedupe_key, metadata)
  ...
  FOR r IN
    SELECT id, nombre, apellido, estado, pause_motivo, pause_fecha_estimada_retorno FROM public.alumnos
  ...
  FOR r IN
    SELECT s.id, s.alumno_id, a.nombre, a.apellido, s.fecha_fin FROM public.suscripciones s
    JOIN public.alumnos a ON a.id = s.alumno_id
