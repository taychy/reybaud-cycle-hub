...
BEGIN
  ...
  -- Token determinístico: hash del estado que importa para revalidar.
  -- Debe reproducirse idénticamente cuando apply_package_change recompute preview
  -- (mismo paquete, precio, cupos, amount_paid, status). Así el token coincide
  -- salvo que algo cambie realmente entre preview y apply.
  v_token := md5(
